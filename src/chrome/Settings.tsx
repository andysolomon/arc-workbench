  ...
  return (
    <div id="wb-settings" role="group" aria-label="display settings" className="wb-settings" data-insp={insp} data-chrome="1" style={{ position: 'absolute', top: 'calc(100% + 8px)', zIndex: 40, width: '238px', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: '10px', boxShadow: 'var(--shadow-panel)', padding: '12px 14px 14px', display: 'flex', flexDirection: 'column', gap: '10px', animation: 'wb-fade var(--motion-fast) ease-out' }}>
      ...
      {ctl.UIOPTS.map(([k, label]) => {
        ...
        return (
          <button key={k} role="switch" aria-checked={on} onClick={() => ctl.setUi(k)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', width: '100%', background: 'none', border: 'none', padding: '2px 0', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: '11px' }}>
            <span>{label}</span>
            <span style={pill} aria-hidden="true"><span style={knob} /></span>
          </button>
        );
      })}
    </div>
